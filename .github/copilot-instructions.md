...
| `writer` | ドキュメント・ルール作成 | 技術文書・.github/ 配下の整備 |
...
```
1. architect に構造評価・配置判断を依頼（大規模変更・新規モジュール時）
2. manager に計画策定を依頼（architect の判断を入力として含む）→ 実行計画を受領
3. 計画に基づき developer に実装を依頼（architect の設計方針を含む）
4. reviewer にレビューを依頼
5. レビュー指摘があれば developer に修正指示を転送
6. LGTM まで 4-5 を繰り返す
